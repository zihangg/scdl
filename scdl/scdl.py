...
def download_hls_mp3(track, title): #download mp3 version of files
    ...
    if arguments['--start'] is not None and arguments['--end'] is None:
        ...
        os.system(
            "ffpb -i {0} -ss {1} -c:v copy {2} -loglevel fatal".format(
                shlex.quote(url),
                start,
        ...
        os.system(
            "ffpb -i {0} -to {1} -c:v copy {2} -loglevel fatal".format(
                shlex.quote(url),
                end,
                shlex.quote(filename)
            )
        ...
        os.system(
            "ffpb -i {0} -ss {1} -to {2} -c:v copy {3} -loglevel fatal".format(
                shlex.quote(url),
                start,
                end,
                shlex.quote(filename)
        ...
        os.system(
            "ffpb -i {0} -c:v copy {1} -loglevel fatal".format(
                shlex.quote(url),
                shlex.quote(filename)
            )
        )
